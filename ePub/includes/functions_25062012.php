...
//ob_start();
	...
	function CreateOpf($k) {
		...
		if($outputtpe == 1) $start = 0;
                ...
                $htmlFolder                                 =               EPUB_FOLDER."/".$c_dirname."/OPS/";
                
		$contentmanifest                            =       '';
                $contentspine                               =       '';
                
                $htmlfilepath                               =		opendir($htmlFolder);
                while ($name                                =		readdir($htmlfilepath)) {    
                    $htmlfirstthree                         =		substr($name,0,3);
                    $htmlfileexten                          =           GetFileExtension($name);
                    $htmlfile                               =           RemoveExtension($name);

                    if ($name != '.' && $name != '..' && $htmlfirstthree == 'pg_' && $htmlfileexten == 'html') {                         
                    $contentmanifest .= "<item id='$htmlfile' href='$name' media-type='application/xhtml+xml'/>\n";
                    $contentspine .= "<itemref idref='$htmlfile' linear='yes'/>\n";
	...
	function CreateNcx($fcount) {
		...
		$contentnmap = '';
                ...
                while ($name                                =		readdir($htmlfilepath)) {    
                    ...
                    if ($name != '.' && $name != '..' && $htmlfirstthree == 'pg_' && $htmlfileexten == 'html') {                         
                    $contentnmap .= "<navPoint id='navpoint-$start' playOrder='$start'>\n"
							. "<navLabel>\n"
								. "<text>$htmlfile</text>\n"
							. "</navLabel>\n"
							. "<content src='$name'/>\n"
