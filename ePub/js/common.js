...
$(document).ready(function(){
   ...
   $('#acteditsubmit').on('click', function() {
	...
	var activitytype            =	$('#activitytype').val();
        var activitydesc            =	$.trim($('#actdesc').val());
   ...
   $('#addsave').on('click', function() {
       ...
       var updateactivity      =        $.ajax({
                ...
                url             : posturl,
                beforeSend      :       function() {            
                    $(" <div />" ).attr("id","showajaxload").addClass("miniajaxaddsave").html('<span id="showmsg" style="display:block;"><img id="pic" src="images/ajax-loader.gif" border="0"/></span>').appendTo($( "body" ));
                    $(" <div />" ).attr("id","backColor").appendTo($( "body" ));        
                    $("#showajaxload").show();
                    $("#backColor").css({"opacity":"0.7"});
                    $("#backColor").show();
   ...
   $('#actsubmit').on('click', function() {       
	...
	var lowpdf                  =	extension.toLowerCase();
        ...
        if( /[^a-zA-Z0-9_-]/.test( activityname ) ) {
        ...
        if($('#actdesc').val() != '') {
            ...
            if( /[^a-zA-Z0-9_-]/.test( activitydesc ) ) {
                alert('Input is not alphanumeric');
                $('#actdesc').val('');
                $('#actdesc').focus();
                return false;
...
function actInstantSubmit() {
    var activityname            =	$.trim($('#InstActivityname').val());
    var activitytype            =	$('#InstActivitytype').val();
    var activityDesc            =	$.trim($('#InstActdesc').val());
...
function actEditSubmit() {
    //alert("211");
    var activityname            =	$.trim($('#activitynameEdit').val());
    var activitytype            =	$('#activitytypeEdit').val();
    var activitydesc            =       $.trim($('#actdescEdit').val());
...
function presetselect() {
    var presetval		=	$.trim($('#presetset').val());
    ...
    if (presetval   ==  "") {
        //alert(1);
//       $("#form1").children('input, select, textarea').each(function() {
//           this.val('');
//       });
//        $("#form1").children('input[type=radio], input[type=checkbox]').each(function()
//        {
//            this.checked = false;
//            // or
...
function checkimage() {
    ...
    if(coimage == '') {
        ...
        alert('Please upload only these image types JPG, JPEG, GIF OR PNG file.');
        $('#uploadFile_div').html($('#uploadFile_div').html());
        //$('#coverimage').val('');
        $('#coverimage').focus();
        return false;
    }
...
function pdfcovert() {
    ...
    var cusfon              =	$("input[value='spetrue']:checked").val();
    var deffon              =	$("input[value='spefal']:checked").val();
    var portonly            =	$("input[value='portrait-only']:checked").val();
    var landonly            =	$("input[value='landscape-only']:checked").val();
    var noneonly            =	$("input[value='none']:checked").val();
    var versiontype         =	$('#versiontype').val();
    var outtype             =	$('#outtype:checked').val();	

            if(versiontype	==	'') {
                    alert("Please select ePub version");
                    $('#versiontype').focus();
                    return false;
            }
            if(!outtype) {
            ...
            if(dpires		!=	'') {
                    if(dpires < 100 ) {
                            ...
                            alert('Please enter only numbers in the Resolution.');
                            $('#dpires').val('');
                            $('#dpires').focus();
                            return false;
                    }
                ...
                if($('#dpires').val() != '') {
                    alert('Please enter DPI in the Resolution.');
                    $('#dpires').val('');
                    $('#dpires').focus();
                    return false;
                }
            ...
            if((supdev == 'nodev') || (supdev == null)) {
                    alert('Please select Supporting Device.');
                    $('#supp').focus();
                    return false;
            } else {
            ...
            if(g > 0) {
                    alert("Please select Orientation Type");
                    $('#oriloc').focus();
                    return false;
            }
            ...
            $("#form1").ajaxForm({
                ...
                error: function (msg, status, e) {
                    if(msg.status == 0) {
                        ...
                        $("#backColor").fadeOut("fast");
                        $("#backColor").hide();
                        alert("Some Data Error in the form");
                        return false;
                    }
                    //alert(status);
                    //alert(e);
                ...
                beforeSend  : function(){
                    ...
                    $(" <div />" ).attr("id","showajaxload").addClass("miniloadsubmit").html('<span id="showmsg" style="display:block;"><img id="pic" src="images/ajax-loader.gif"  border="0"/></span>').appendTo($( "body" ));
                    $(" <div />" ).attr("id","backColor").appendTo($( "body" ));        
                    $("#showajaxload").show();
                    $("#backColor").css({"opacity":"0.7"});
                    $("#backColor").show();                    
...
function checkpdf() {
    ...
    if(lowpdf == '') {
        ...
        alert('Please upload only pdf files.');
        $('#upload_pdf').html($('#upload_pdf').html());
        $('#pdffile').focus();
        return false;
    }    
}

function addsaveepub() {
    //alert("good");    
    var htmlandactfiles     =       '';
    var k                   =       1;    
    var w                   =       $('#boxes li').length;
    //return false;
    $('#boxes li').each(function() {        
        htmlandactfiles += $(this).attr('linewid');
        if(k<w) {    
            htmlandactfiles += ';';
        }
        k++;
    ...
    var updateactivity      =        $.ajax({
                ...
                data            : {'insert_id' : insert_id, 'totalfiles' : htmlandactfiles },
                cache		: false,
                async		: false,
                dataType	: "text"
            }).responseText;
...
function swappreviewshow(epubFolder,serverPath,settingId,preOne,nextone,htmlFullPath,firstOne,lastOne) {    
    alert(epubFolder);
    alert(serverPath);
    alert(settingId);
    //$('#previewid').remove();
