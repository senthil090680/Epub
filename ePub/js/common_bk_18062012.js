...
$(document).ready(function(){
       ...
       if(activityselect == '') {
           ...
           return false;
            ...
            var getactivitylist      =       $.ajax({
                ...
                url             : posturl,
                beforeSend      :       function() {            
                    $(" <div />" ).attr("id","showajaxload").addClass("miniajaxloading").html('<span id="showmsg" style="display:block;"><img id="pic" src="images/ajax-loader.gif" width="220" height="120" border="0"/></span>').appendTo($( "body" ));
   ...
   $('#addsave').on('click', function() {
       ...
       var updateactivity      =       $.ajax({
                ...
                url             : posturl,
                beforeSend      :       function() {            
                    $(" <div />" ).attr("id","showajaxload").addClass("miniajaxupdate").html('<span id="showmsg" style="display:block;"><img id="pic" src="images/ajax-loader.gif" width="220" height="120" border="0"/></span>').appendTo($( "body" ));
                    $(" <div />" ).attr("id","backColor").appendTo($( "body" ));        
                    $("#showajaxload").show();
                    $("#backColor").css({"opacity":"0.7"});
                    $("#backColor").show();
	...
	var outtype		=	$('#outtype:checked').val();	
		...
		$("#form1").ajaxForm({
                    success:function(msg){
                        ...
                        $(" <div />" ).attr("id","showmsgid").addClass("miniload").html('<span id="showmsg" style="display:block;"><img id="pic" src="images/bigrotation2.gif" width="50" height="50" border="0"/></span>').appendTo($( "body" ));
                        
                        $("#showmsgid").css("display","block");			
...
$('#previewid').css("display","block");   
    ...
    $('#'+htmlid).html('<iframe id="newiframe" width="600px" height="500px" style="overflow:auto;" frameborder="0" src="'+newsrc+'"></iframe>');
        ...
        start : function(e, ui) {
                $("#htmlPopBox").droppable({
                        drop : function handleCardDrop( event, ui ) {
                                ...
                                $('<li id="'+htmlnoext+'" class="htmlPage '+htmlnoext+'"><span id="'+serverpath+activpath+activityid+'_'+substr[1]+'/'+'" ></span><div class="pgEditDelete"><img onclick="deleteAddActivity(\''+activityid+'_'+substr[1]+'\',\''+activpath+'\',\''+htmlnoext+'\');" src="images/page_delete.png"/></div><a id="'+substr[0]+'" onclick="popupclick(\''+substr[0]+'\',\''+serverpath+activpath+activityid+'_'+substr[1]+'/'+'\')">'+shortname+'...</a></li>').appendTo('.htmlPageList');
                        }
                });
            }
        });
...
function actpreviewShow (activityId,serverPath) {
    ...
    $(" <div />" ).attr("id","previewid").addClass("minihtml").html('<div class="closepbox"><a href="javascript:void(0)" onclick="javascript:return closePrivateConfirm(this,\''+htmlid+'\');"><img src="images/pop-close-small.png" /></a></div><br/><br/><br/><div style="display:block;display:inline;padding-left:20px;padding-right:80px;" id="'+htmlbox+'"></div>').appendTo($( "body" ));   
    $('#previewid').css("display","block");
    
    $('#'+htmlbox).load(serverPath+responseValue, function() {
